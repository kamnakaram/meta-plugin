---
name: meta-test
description: "A/B test design and experiment planning for Meta ads. Structured hypothesis framework, statistical significance calculator, test duration estimator, sample size calculator, and Meta Experiments setup (A/B, Holdout, Conversion Lift, Brand Survey). Use when user says A/B test, split test, experiment design, Meta Experiments, test hypothesis, statistical significance, sample size, or test duration."
user-invokable: true
...
Learning phase: Meta ad sets typically 3-7 days (≈50 events) to exit; don't read a test before cells exit learning
...
## Meta Experiments Setup

Use **Ads Manager → Experiments** (the A/B Test tool) rather than manually
duplicating ad sets — Meta splits the audience so the variants never overlap
in the auction, which manual duplication cannot guarantee.

### Test types
- **A/B Test**: compare two ads, ad sets, or campaigns on one variable
  (creative, audience, placement, optimization event, or bidding)
- **Holdout / Conversion Lift**: a randomized holdout group sees no ads, giving
  a true incremental read on conversions and revenue (the gold standard for
  "did the ads actually cause this?")
- **Brand Survey Lift**: for awareness/consideration objectives, measures
  ad-recall and brand lift via polled holdout vs exposed groups

### Setup rules
- **Isolate one variable.** Everything except the tested element must be identical
  across cells, or the result is uninterpretable.
- **Split budget evenly** across cells; a practical floor is ~$100/day per cell so
  each exits the learning phase and reaches significance in a reasonable window.
- **Pick the primary metric before launch** (CPA, ROAS, cost per result) — Meta
  declares a winner at 95% confidence on that metric.
- **Don't edit mid-test.** Budget/audience/creative edits reset the learning phase
  and contaminate the comparison.
- **Duration**: 7-14 days typical; run at least 7 days to capture a full weekly
  cycle, and long enough to hit the sample size below.
- **Mind attribution.** Let the attribution window (e.g. 7-day click) close before
  reading results, and keep the same window across cells.

### Reading the result
- Meta reports the probability each cell is the winner plus the cost-per-result
  range; require the primary-metric win to be significant AND to hold for a few
  days before rolling the winner out.
- For a Holdout, judge on **incremental** conversions/revenue, not in-platform
  attributed results — that's the whole point of the holdout.

## What to Test (Priority Order)

### High Impact (test first)
1. **Creative concept** (different messaging angles, not just color changes)
2. **Hook / first 1-2 seconds** (video opening on Feed & Reels)
3. **Offer structure** (pricing, discount type, free trial length)
4. **Landing page** (headline, CTA, form length)
5. **Bidding strategy** (Highest Volume vs Cost Cap vs ROAS goal)
...
| Parameter | Value |
|-----------|-------|
| Objective | [Sales / Leads / App / Traffic / Awareness] |
