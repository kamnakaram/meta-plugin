...
def parse_markdown(filepath: str) -> dict:
    ...
    for line in lines:
        ...
        # Pillar scores: handle both "Pixel / CAPI Health: 78/100" and table rows.
        # Use a pattern that can cross table pipes.
        m = re.search(
            r"(Pixel\s*/\s*CAPI(?:\s*Health)?|Creative|Account\s+Structure|Structure|"
            r"Audience(?:\s*&\s*Targeting)?|Attribution|CAPI|Tracking|Budget|Compliance)"
            r".*?(\d{1,3})\s*/\s*100",
            line, re.I
        )
        if m:
            data["platform_scores"][m.group(1).strip()] = int(m.group(2))
...
def _add_page_footer(canvas, doc):
    ...
    canvas.drawCentredString(
        letter[0] / 2, 0.4 * inch,
        f"meta-ads v{__version__}  |  Page {doc.page}  |  {datetime.now().strftime('%B %d, %Y')}"
    )
    canvas.restoreState()
...
def build_pdf(data: dict, output_path: str, brand_name: str = ""):
    ...
    elements.append(Paragraph(
        f"Generated {datetime.now().strftime('%B %d, %Y')}  |  Powered by meta-ads v{__version__}",
        styles["RSubtitle"],
    ))
    ...
    elements.append(Spacer(1, 6))
    elements.append(Paragraph(
        f"Report generated by meta-ads v{__version__}  |  "
        "https://github.com/AI-Marketing-Hub/meta-ads",
        styles["RFooter"],
    ))
