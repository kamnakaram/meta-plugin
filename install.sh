...
REPO_URL="https://github.com/AI-Marketing-Hub/meta-ads"
...
resolve_target_paths() {
    ...
    case "$target" in
        ...
        codex)
            ...
            ;;
        cursor)
            SKILL_BASE="${HOME}/.cursor/extensions/meta-ads/skills"
            AGENT_DIR="${HOME}/.cursor/extensions/meta-ads/agents"
            ALLOW_PIP=0
        ...
        windsurf)
            ...
            ;;
        gemini)
            SKILL_BASE="${HOME}/.gemini/extensions/meta-ads/skills"
            AGENT_DIR="${HOME}/.gemini/extensions/meta-ads/agents"
            ALLOW_PIP=0
...
main() {
    ...
    local SKILL_DIR="${SKILL_BASE}/meta-ads"

    echo "════════════════════════════════════════"
    echo "║   Meta Ads - Installer               ║"
    ...
    echo "↓ Downloading Meta Ads..."
    git clone --depth 1 "${REPO_URL}" "${TEMP_DIR}/meta-ads" 2>/dev/null

    # Copy main skill + references
    echo "→ Installing skill files..."
    cp "${TEMP_DIR}/meta-ads/meta-ads/SKILL.md" "${SKILL_DIR}/SKILL.md"
    cp "${TEMP_DIR}/meta-ads/meta-ads/references/"*.md "${SKILL_DIR}/references/"

    # Copy sub-skills
    echo "→ Installing sub-skills..."
    for skill_dir in "${TEMP_DIR}/meta-ads/skills"/*/; do
    ...
    cp "${TEMP_DIR}/meta-ads/agents/"*.md "${AGENT_DIR}/" 2>/dev/null || true

    # Copy scripts (optional Python tools)
    SCRIPTS_DIR="${SKILL_DIR}/scripts"
    if [ -d "${TEMP_DIR}/meta-ads/scripts" ]; then
        echo "→ Installing Python scripts..."
        mkdir -p "${SCRIPTS_DIR}"
        cp "${TEMP_DIR}/meta-ads/scripts/"*.py "${SCRIPTS_DIR}/"
        cp "${TEMP_DIR}/meta-ads/requirements.txt" "${SKILL_DIR}/requirements.txt"
    fi
    ...
    if [ -d "${SKILL_BASE}/banana" ] || [ -f "${SKILL_BASE}/banana/SKILL.md" ]; then
        ...
        echo "  ⚠ banana-claude not installed. Image generation (/meta-ads generate, /meta-ads photoshoot) requires it."
    ...
    echo ""
    echo "✓ Meta Ads installed successfully for ${HOST_LABEL}!"
    echo ""
    echo "  Installed to:"
    ...
    echo ""
    echo "  Bundled:"
    echo "    • 1 main skill (meta-ads orchestrator)"
    echo "    • 15 sub-skills (audit + functional + creative)"
    echo "    • 9 agents (5 audit + 4 creative)"
    echo "    • 15 reference files"
    echo "    • 12 industry templates"
    echo ""
    echo "Usage:"
    echo "  1. Start your host CLI"
    echo "  2. Run commands:       /meta-ads audit"
    echo "                         /meta-ads plan saas"
    echo "                         /meta-ads analyze"
    echo ""
