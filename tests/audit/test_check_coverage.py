...
1. **No orphans in the catalog** — every check ID in
   ...
   audit reference file under ``meta-ads/references/``.
...
def test_total_check_count_is_at_least_50(check_catalog):
    """Sanity: the Meta catalog pins 50 tracked checks (M01-M40 + hyphenated
    v1.5+ IDs). This baseline fires immediately if a future regression drops
    checks from the Meta catalog."""
    total = sum(len(p["check_ids"]) for p in check_catalog["platforms"].values())
    assert total >= 50, f"Meta catalog totals {total}; baseline is 50"
